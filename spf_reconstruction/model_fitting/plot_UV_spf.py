...
from correlator_analysis.double_extrapolation.BB_renormalization.compute_Z import (
    get_scale_choices,
)
...
class SpfPlotter:
    ...
    def __setup_plot(self):
        self.fig, self.spf_ax, _ = lpd.create_figure(
            xlabel=r"$\omega/T$",
    ...
    def _plot_BB(self):
        ...
        for scale_choice in scale_choices:
            if scale_choice.choice_label == "ref6.28_UVNLO_IRLO":
                self.BB = np.loadtxt(
                    self.inputfolder
                    + "/BB/BB_flow_extr_relflow_"
                    + scale_choice.choice_label
                    + ".txt",
                    unpack=True,
                )
    ...
    def _plot_IR_part(self):
        ...
        self.corr_ax.errorbar(
            np.arange(1 / 36, 0.501, 1 / 36),
            self.corr_IR - self.norm_value_IR,
            color="pink",
            fmt="-",
        )
    ...
    def _plot(self):
        ...
        for uv_spf_data in self.uv_spf_data_list:
            ...
            self.spf_ax.errorbar(
                uv_spf_data.OmegaByT_arr,
                twoRhoByomegaTsq(uv_spf_data.OmegaByT_arr, uv_spf_data.PhiUVByT3),
                fmt=uv_spf_data.input_params.fmt,
                label=uv_spf_data.input_params.plotlabel,
            )
            self.corr_ax.errorbar(
                np.arange(1 / 36, 0.501, 1 / 36),
...
def compute_correlator(OmegaByT_arr, SpfByT3_arr):
    def calc_corr(OmegaByT_arr, SpfByT3_arr, tauT):
        integrand = scipy.interpolate.InterpolatedUnivariateSpline(
            OmegaByT_arr,
            [
                Integrand(OmegaByT, SpfByT3, tauT)
                for OmegaByT, SpfByT3 in zip(OmegaByT_arr, SpfByT3_arr)
            ],
            k=3,
            ext=2,
        )
        corr = scipy.integrate.quad(integrand, OmegaByT_arr[0], OmegaByT_arr[-1])[
            0
        ] / spf_reconstruct.Gnorm(tauT)
        return corr

    def Integrand(OmegaByT, SpfByT3, tauT):
        return 1.0 / np.pi * spf_reconstruct.Kernel(OmegaByT, tauT) * SpfByT3

    Nt = 36

    correlator = np.asarray(
        [
            calc_corr(OmegaByT_arr, SpfByT3_arr, tauT)
            for tauT in np.arange(1 / Nt, 0.501, 1 / Nt)
        ]
    )
...
def get_spf_and_corr(Nf, T_in_GeV, Npoints, Nloop, input_params):
    OmegaByT_arr, g2_arr, SpfByT3_arr = compute_UV_spf.get_spf(
        Nf,
        input_params.max_type,
        input_params.min_scale,
        T_in_GeV,
        input_params.omega_prefactor,
        Npoints,
        Nloop,
        input_params.order,
        input_params.corr,
...
def main():
    ...
    uv_spf_data = lpd.parallel_function_eval(
        parallelwrapper, input_params, 10, Nf, T_in_GeV, Npoints, Nloop
    )
