...
args="--nknots 1 2 3 --order 2 --constraints 0 2 0 0.5 1 0 --randomization-factor 0 --nsamples 1000 --base-point=0.31 --xmax=0.5 --data-input sample --method from_sample  --no-tex --plot-xmax 0.5 --folder=$path1/continuum_limit/ --log-level INFO"
#--nknots 3
...
$extrapolate --xmin=\`bc <<< "scale=5; \${i}/sqrt(8*0.014)+0.02"\` --plot-xmin=\`bc <<< "scale=5; \${i}/sqrt(8*0.014)+0.02"\` $args --outname=EE_\${i} $path1/s*t*_b*/single_flow/EE_\${i}_Nt*_btstrp_samples.dat >> $path1/continuum_limit/logs/\${SLURM_JOB_NAME}_\${i}.out &
done
wait
