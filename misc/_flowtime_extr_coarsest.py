...
def main():
    ...
    qcdtype = "quenched_1.5Tc_zeuthenflow"
    corr = "EE"
    conftype = "s064t16_b0687361"
    
    flow_radius = numpy.loadtxt(lpd.get_merged_data_path(qcdtype,corr,conftype)+"/flowradii_s064t16_b0687361.dat")
    
    nflow = len(flow_radius)
    ...
    ylabels = [r'$\displaystyle\frac{G^{\begin{subarray}{l}\mathrlap{\textrm{\tiny  Zeuthen}}\\[-0.3ex] \textrm{\tiny latt}\end{subarray}}_\tau {\scriptstyle(\tau_\mathrm{F})}}{G_{\tau,\tau_\mathrm{F}=0}^{\begin{subarray}{l}\mathrlap{\textrm{\tiny  norm}}\\[-0.4ex] \textrm{\tiny latt}\end{subarray}}  } $',
               r'$\displaystyle\frac{G^{\begin{subarray}{l}\mathrlap{\textrm{\tiny  Wilson}}\\[-0.3ex] \textrm{\tiny latt}\end{subarray}}_\tau {\scriptstyle(\tau_\mathrm{F})}}{G_{\tau}^{\begin{subarray}{l}\mathrlap{\textrm{\tiny  norm}}\\[-0.4ex] \textrm{\tiny latt}\end{subarray}} {\scriptstyle(\tau_\mathrm{F})}} $']
    
    for qcdtype,ylabel in zip(qcdtypes, ylabels):
        fig, ax, plots = lp.create_figure(xlims=[-0.0001,0.0023], ylims=[ymin,ymax], xlabel=r'$\tau_\mathrm{F} T^2$', ylabel=ylabel, 
                                            xlabelpos=(0.95,0.05), ylabelpos=(0.08,0.96), UseTex = UseTex)
        ...
        paths = [inputfolder+"/EE_"+conftype+".dat", 
                 ...
                 outputfolder+"/EE_"+conftype+"_flow_extr_quality_"+flowstr+".pdf", 
                 outputfolder+"/EE_"+conftype+"_flow_extr_ratio.pdf",
                 inputfolder+"/EE_"+conftype+"_flow_extr_slope.txt"]
        ...
        with open(paths[2], 'w') as outfile:
            ...
            with open(paths[6], 'w') as outfile_slope:
                outfile_slope.write('# flowtime zero extrapolation slope for all tauT \n')
                ...
                for i,tauT in enumerate(lpd.tauT_imp[max_nt]):
                    ...
                    if not numpy.isnan(flowstart):
                        ...
                        flowend = min((numpy.abs(flow_radius - maxflowtime)).argmin(), 134)
                        xdatatmp = flow_radius[flowstart:flowend]**2 /8
    ...
    ylabel = r'$\displaystyle \frac{\left.G^{\begin{subarray}{l}\mathrlap{\textrm{\tiny  Wilson}}\\[-0.3ex] \textrm{\tiny latt}\end{subarray}}{\scriptstyle(\tau,\tau_\mathrm{F})}/G^{\begin{subarray}{l}\mathrlap{\textrm{\tiny  norm}}\\[-0.4ex] \textrm{\tiny latt}\end{subarray}}{\scriptstyle(\tau, \tau_\mathrm{F})} \right|_{\tau_\mathrm{F} \rightarrow 0} }{\left.G^{\begin{subarray}{l}\mathrlap{\textrm{\tiny  Zeuthen}}\\[-0.3ex] \textrm{\tiny latt}\end{subarray}}{\scriptstyle(\tau,\tau_\mathrm{F})}/G_{ {\tau}_{ F }=0}^{\begin{subarray}{l}\mathrlap{\textrm{\tiny  norm}}\\[-0.4ex] \textrm{\tiny latt}\end{subarray}} {\scriptstyle(\tau)}\right|_{\tau_\mathrm{F} \rightarrow 0} } $'
