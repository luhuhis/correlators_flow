...
def plot_fig_tauT_on_xaxis(args):
    fig, ax, axtwiny = lpd.create_figure(
        ylabelpos=(0.15, 0.98),
        ylims=[0.5, 50000],
        xlims=[0, 0.52],
        ylabel=r"$\displaystyle \frac{G_E^\mathrm{LO}}{(g^2 C_F)T^4}$",
        xlabel=r"$\tau T$",
    )
    ax.set_yscale("log")
    ax.minorticks_off()
    ...
    EE_latt = numpy.loadtxt(
        args.inputfolder + "EE_latt_flow_" + str(args.Ntau) + ".dat"
    )
    EE_cont = numpy.loadtxt(
        args.inputfolder + "EE_cont_flow_" + str(args.Ntau) + ".dat"
    )
    ntauTcont = len(
        numpy.arange(
            1 / args.Ntau / contCorrResolution,
            args.Ntau / 2 + 1 / args.Ntau / contCorrResolution / 2,
            1 / args.Ntau / contCorrResolution,
        )
    )

    plotstyle_points = dict(linewidth=1, markersize=6, fillstyle="none")
    for i, fmtlat, fmtcont, color in zip(
        (0, 50, 100), ("x", "+", "o"), ("-", "--", ":"), ("k", "C0", "C1")
    ):
        ...
        plots.append(
            ax.errorbar(
                [row[0] for row in EE_cont[j0:j1]],
                [row[2] for row in EE_cont[j0:j1]],
                fmt=fmtcont,
                color=color,
                **plotstyle_points,
                label=flowstr,
        ...
        plots.append(
            ax.errorbar(
                [row[0] for row in EE_latt[i0:i1]],
                [row[2] for row in EE_latt[i0:i1]],
                fmt=fmtlat,
                color=color,
                **plotstyle_points,
                label=flowstr,
        ...
        xval = numpy.abs(
            flow_radius / numpy.sqrt(8 * 0.014) - EE_cont[j0:j1, 0]
        ).argmin()
...
def plot_fig_flow_on_xaxis(args):
    ...
    for prefix, fmt, subtitle in zip(
        ["EE_cont_flow_", "EE_latt_flow_"], ["-", "--"], ["cont.", "latt."]
    ):
        ...
        data = pd.read_csv(
            file_path, delim_whitespace=True, header=None, names=["x", "y", "value"]
        )
