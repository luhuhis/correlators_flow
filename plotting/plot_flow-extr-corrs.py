...
def main():
    ...
    # TODO: abstract this in a function such that you can input any corr. or maybe make it an input arg?
    ...
    for f, flowradiusBytauT in enumerate(args.flowradiusBytauT):
        ...
        ax2.errorbar(frankenstein_xdata, frankenstein_ydata, frankenstein_edata, zorder=-len(args.flowradiusBytauT)+f,
                     **lpd.chmap(lpd.plotstyle_add_point, markersize=0, fmt='x-', color=lpd.get_color(args.flowradiusBytauT, f),
                                 label=r'$'+r'{0:.3f}'.format(flowradiusBytauT)+r'}$'))  #
    ...
    ax2.legend(title=r'$G_{\sqrt{8\tau_F}/\tau=}$', **lpd.chmap(lpd.legendstyle, bbox_to_anchor=(1, 0.5), loc='center left', ncol=1, fontsize=7))
