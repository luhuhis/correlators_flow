...
for i,fmtlat,fmtcont,color in zip((0,50,100), ('+','x','+'), ('-','--',':'), ('k','C0','C1')):
    i0=int(i*(Ntau/2))
    i1=int((i+1)*(Ntau/2))
    j0=int(i*ntauTcont)
    ...
    plots.append(ax.errorbar([row[0] for row in EE_cont[j0:j1]], [row[2] for row in EE_cont[j0:j1]], fmt=fmtcont, color=color, **plotstyle_points, label=flowstr, zorder=-1))
    if i == 0:
        plots.append(ax.errorbar([row[0] for row in EE_latt[i0:i1]], [row[2] for row in EE_latt[i0:i1]], fmt=fmtlat, color=color, **plotstyle_points, label=flowstr, zorder=-2))
