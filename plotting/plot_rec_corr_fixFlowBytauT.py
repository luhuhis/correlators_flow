...
def find_and_plot_and_save_frankenstein(flowradiusBytauT_list, possible_tauTs, y_int_list, e_int_list, out_file_path, ax, color_list, Glabel):
    ...
    for f, flowradiusBytauT in enumerate(flowradiusBytauT_list):
        ...
        ax.errorbar(frankenstein_xdata, frankenstein_ydata, frankenstein_edata, zorder=-len(flowradiusBytauT_list)+f,
                    **lpd.chmap(lpd.plotstyle_add_point, markersize=0, elinewidth=plotwidth, mew=plotwidth, fmt='x', color=color_list[f],
...
def main():
    ...
    fitparams = []
    just_UVs = []
    for fitparam_file, PhiUV_file in zip(args.fitparam_files, itertools.cycle(args.PhiUV_files)):
    ...
    fig, ax, _ = lpd.create_figure(xlims=args.xlims, ylims=args.ylims, xlabel=r'$\tau T$', xlabelpos=(0.95, 0.05), ylabelpos=(0.08, 0.9),
                                   UseTex=args.use_tex) # ylabel=r'$\frac{G}{ G^\mathrm{norm} }$', (1.5 * (3 + 3 / 8), 1.5 * (3 + 3 / 8) / 16 * 9)
    ...
    if args.qcdtype is not None and args.corr is not None and args.conftype is not None:
        for qcdtype, corr, conftype, color in itertools.zip_longest(args.qcdtype, args.corr, args.conftype, colors):
            if qcdtype is None:
    ...
    ax.legend(**lpd.chmap(lpd.legendstyle, framealpha=0, bbox_to_anchor=(0.02, 1), loc='upper left', ncol=2, columnspacing=1.2, fontsize=7))  # title=r'$G=$'
