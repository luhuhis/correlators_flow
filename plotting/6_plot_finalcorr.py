...
fig, ax, plots = pl.create_figure(xlims=[0.05,0.51], ylims=[1,4], xlabel=r'$\tau T$', ylabel=r'$\displaystyle \frac{G^\mathrm{ cont }(\tau)}{G^{\mathrm{norm } }(\tau)}$')
...
plots.append(ax.errorbar(EE_cont_2015_new[:,0], EE_cont_2015_new[:,1], EE_cont_2015_new[:,2], color=matplotlib.cm.gnuplot(0.25), alpha=1, **pl.plotstyle_add_point, label=r'Multi-level method$^{1,2}$'+"\n"+r'w/ $2016$ renorm. update'))#\beta_g (\mathcal{Z}_\mathrm{pert} -1)/6 \approx 0.138
