...
class ScaleFunctions:
    ...
    def get_muF_by_T_where_we_measure(cls):
        muF_by_T_where_we_measure = np.sort(
            np.unique(
                np.concatenate(
                    [
                        cls.convert_sqrt8taufTByTau_to_muFByT(
                            lpd.get_relflow_range(), tauT
                        )
...
class GeneralParameters:
    ...
    muF_by_T_where_we_measure: np.ndarray


@lpd.typed_frozen_data
class RawDataContainer:
    muF_by_T_arr: np.ndarray
    g2_arr: np.ndarray
    g2_err_arr: np.ndarray
    ...
    target_muF_by_T_for_cont_extr: np.ndarray


@lpd.typed_frozen_data
class ContDataContainer:
    muF_by_T_cont: np.ndarray
    g2_latt_from_int: np.ndarray  # Shape["* nflow, * nNts"]
    g2_latt_from_int_err: np.ndarray  # Shape["* nflow, * nNts"]
    g2_cont: np.ndarray  # Shape["* nflow, [g2, g2err]"]
    g2_extr_slope: np.ndarray  # Shape["* nflow, [slope, slope_err]"]
    g2_extr_chisqdof: np.ndarray  # Shape["* nflow, [chisqdof, chisqdof_err]"]
    g2_cont_int: scipy.interpolate._fitpack2.InterpolatedUnivariateSpline


@lpd.typed_frozen_data
class MSBarDataContainer:
    mubar_by_T: np.ndarray
...
def plot_g2_vs_1overNt2(args, general_params, cont_data_container):
    ...
    fig3, ax3, _ = lpd.create_figure(
        xlabel=xlabel,
        ylabel=r"$ \displaystyle \alpha_\text{flow}$",
        ylims=(0.95 / (np.pi * 4), 3.55 / (np.pi * 4)),
    )
    ...
    for j, muF_by_T in enumerate(plot_muF_by_T):
        ...
        ax3.errorbar(
            np.insert(1 / np.asarray(args.Nts) ** general_params.Ntexp, 0, 0),
            np.insert(
                cont_data_container.g2_latt_from_int[i] / (np.pi * 4),
                0,
                cont_data_container.g2_cont[i, 0] / (np.pi * 4),
            ),
            np.insert(
                cont_data_container.g2_latt_from_int_err[i] / (np.pi * 4),
                0,
                cont_data_container.g2_cont[i, 1] / (np.pi * 4),
            ),
            color=color,
            fmt="|",
            label=lpd.format_float(cont_data_container.muF_by_T_cont[i], 2),
            zorder=-i,
        )
        ax3.errorbar(
            xpoints,
            extrapolation_ansatz(
                xpoints,
                cont_data_container.g2_extr_slope[i, 0]
                * general_params.factor
                / (np.pi * 4),
                cont_data_container.g2_cont[i, 0] / (np.pi * 4),
            ),
            **lpd.fitlinestyle,
        ...
        # counter += 1
    ax3.legend(
        title=r"$\displaystyle \mu_\mathrm{F}/ T$",
        loc="center left",
...
class Plotter_g2_vs_mu:
    def __init__(
        ...
        self.list_of_pertRun_MSBar_data_container: List[PertRunMSBarDataContainer] = (
            list_of_pertRun_MSBar_data_container
        )
    ...
    def __setup_plot(self):
        self.fig, self.ax, _ = lpd.create_figure(
            xlabel=r"$\displaystyle \mu_{\mathrm{F}} / T$",
            ylabel=None,
            ylims=(0, 4.5 / (np.pi * 4)),
            xlims=(1, 200),
        )
    ...
    def __plot_pert_g2(self, mus, suffix="pert"):
        g2s = np.asarray(
            [
                lpd.get_g2_pert(mu * self.general_params.T_in_GeV, Nf=0, Nloop=3)
                for mu in mus
            ]
        )
        self.ax.errorbar(
            ...
            lw=self.linewidth,
            markersize=0,
            label=r"pert. ($\mu = \mu_\mathrm{F} \equiv 1/\sqrt{8\tau_F} $)",
            zorder=-1,
        )
        lpd.save_columns_to_file(
            self.args.outputpath_data + "g2_muF_" + suffix + ".txt",
            (mus, g2s),
            ["mu_F/T", "g2s"],
        )
    ...
    def __plot_lattice_data(self):
        for i, Nt in enumerate(self.args.Nts):
            self.ax.errorbar(
                self.raw_data_container.muF_by_T_arr[i],
                self.raw_data_container.g2_arr[i] / (np.pi * 4),
                fmt=":",
                lw=self.linewidth * 0.75,
                markersize=0,
                label=r"$\alpha_\text{flow}$ ($N_\tau = " + str(Nt) + r"$)",
                zorder=-i - 10,
            )

    def __plot_grey_flow_extr_band(self):
        self.ax.axvline(
            self.general_params.muUV_by_T_NLO,
            alpha=0.5,
            dashes=(2, 2),
            zorder=-10000,
            lw=self.linewidth / 2,
        ...
        self.ax.axvline(
            self.general_params.muUV_by_T_LO,
            alpha=0.5,
            dashes=(2, 2),
            zorder=-10000,
            lw=self.linewidth / 2,
        ...
        self.ax.fill_between(
            [
                self.general_params.min_muF_by_T_in_flow_extr,
                self.general_params.max_muF_by_T_in_flow_extr,
            ],
            [-1, -1],
            [100, 100],
            facecolor="k",
    ...
    def __plot_cont(self):
        ...
        self.ax.errorbar(
            self.cont_data_container.muF_by_T_cont,
            cont_data / (np.pi * 4),
            fmt="-",
            markersize=0,
            lw=self.linewidth,
            label=r"$\alpha_\text{flow}$ (cont.\ extr.)",
            zorder=-1,
        )
    ...
    def __plot_pert_run_from_converted_g2MSbar(self):
        for data in self.list_of_pertRun_MSBar_data_container:
            self.ax.errorbar(
                data.mubar_by_T,
                data.g2_MSBAR_pert_run(data.mubar_by_T) / (np.pi * 4),
                fmt="-",
                markersize=0,
                lw=self.linewidth,
                zorder=(int(data.mubar_by_T[0])) + 20,
                label=r"\begin{flushleft}$\alpha_s$ via Eq.\,"
                + self.args.eq_no
                + r" at \newline  "
                + r" ${\mu_\text{F}}=\bar{\mu}_\mathrm{ref}"
                r"$, then \newline run via "
                + str(data.nloop)
                + r"-loop $\beta$-fct. \end{flushleft}",
            )
            self.ax.axvline(
                x=data.reference_muF_by_T,
                alpha=1,
                dashes=(1, 1),
                zorder=-10000,
                lw=self.linewidth / 2,
    ...
    def __plot_MSBAR(self):
        self.ax.errorbar(
            self.msbar_data_container.mubar_by_T,
            self.msbar_data_container.g2_MSBAR_spline(
                self.msbar_data_container.mubar_by_T
            )
            / (np.pi * 4),
            fmt="-",
            markersize=0,
            lw=self.linewidth,
            label=r"$\alpha_s$ via Eq.\," + self.args.eq_no,
        )
...
class ContinuumExtrapolator:
    ...
    def __get_interpolated_lattice_data(self):
        for i, muF_by_T in enumerate(
            self.raw_data_container.target_muF_by_T_for_cont_extr
        ):
            self.g2_latt_from_int[i] = [
                spline(muF_by_T) for spline in self.raw_data_container.g2_ints
            ]
            self.g2_latt_from_int_err[i] = [
                spline(muF_by_T) for spline in self.raw_data_container.g2_err_ints
            ]
        print(
            "max relative error in % in the lattice data:",
    ...
    def __load_cont_extr_from_disk(self):
        try:
            _, b, berr, m, merr, chisqdof, chisqdof_err = np.loadtxt(
                self.output_file, unpack=True
            )
    ...
    def _fit_sample(cls, ydata, xdata, edata):
        def chisqdof_func(fitparams_, xdata_, ydata_, edata_):
            return np.sum(
                ((ydata_ - extrapolation_ansatz(xdata_, *fitparams_)) / edata_) ** 2
            ) / (len(ydata_) - len(fitparams_))

        fitparams = scipy.optimize.minimize(
            chisqdof_func, x0=np.asarray([-1, 2]), args=(xdata, ydata, edata)
        )
    ...
    def __do_cont_extr(self):
        for i in range(self.nflow):
            fitparams, fitparams_err = bootstr.bootstr_from_gauss(
                ContinuumExtrapolator._fit_sample,
                data=self.g2_latt_from_int[i],
                ...
                args=[
                    1
                    / np.asarray(self.Nts) ** self.general_parameters.Ntexp
                    * self.general_parameters.factor,
                    self.g2_latt_from_int_err[i],
                ],
                parallelize=True,
                nproc=20,
            )  # TODO make nproc an input parameter
    ...
    def __get_cont_data(self):
        ...
        self.cont_data = ContDataContainer(
            self.raw_data_container.target_muF_by_T_for_cont_extr,
...
def load_data_and_interpolate(
    args: argparse.Namespace, general_params: GeneralParameters
) -> RawDataContainer:
    ...
    for i in range(len(args.input_files)):
        tauFbya2, tauF2E, tauF2E_err = np.loadtxt(
            args.input_basepath + "/" + args.input_files[i], unpack=True
        )
    ...
    for i in range(len(args.input_files)):
        ...
        g2_ints.append(
            scipy.interpolate.InterpolatedUnivariateSpline(
                muF_by_T, thisg2, k=order, ext=2
            )
        )
        g2_err_ints.append(
            scipy.interpolate.InterpolatedUnivariateSpline(
                muF_by_T, thisg2_err, k=order, ext=2
            )
        )

    target_muF_by_T_for_cont_extr = np.sort(
        np.unique(
            np.concatenate(
                (
                    np.geomspace(
                        largest_min_muF_by_T, general_params.muUV_by_T_NLO, 500
                    ),
    ...
    return RawDataContainer(
        np.asarray(muF_by_T_arr),
        np.asarray(g2_arr),
        np.asarray(g2_err_arr),
        g2_ints,
        g2_err_ints,
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        type=str,
        default="20",
        help="Equation number for matching in alphas vs mu plot.",
    )
    args = parser.parse_args()

    if len(args.betas) != len(args.input_files) or len(args.Nts) != len(
        args.input_files
    ):
...
def convert_g2_flow_to_MSBAR(g2_flow: float, nf: int = 0) -> float:
    ...
    real_solution = [
        ...
        if np.isreal(sol) and sol >= 0 and sol * 4 * np.pi <= 5
    ][0]
...
def calc_g2_MSBAR(
    cont_data_container: ContDataContainer, target_muF_by_T_for_cont_extr
) -> MSBarDataContainer:
    g2_MSBAR = np.asarray(
        [
            convert_g2_flow_to_MSBAR(cont_data_container.g2_cont_int(mu_by_T))
            for mu_by_T in target_muF_by_T_for_cont_extr
        ]
    )
    g2_MSBAR_spline = scipy.interpolate.InterpolatedUnivariateSpline(
        target_muF_by_T_for_cont_extr, g2_MSBAR, k=3, ext=2
    )
...
class BetaFunction:
    def __init__(self, args, MSBar_data_container, general_params):
        ...
        self.min_muF_by_T_where_we_measure = np.min(
            general_params.muF_by_T_where_we_measure
        )
    ...
    def calc_pert_run_of_g2_MSBAR(self):
        muF_by_T_targets_extra = np.geomspace(
            np.max(self.MSBar_data_container.mubar_by_T), 220, 200
        )
        target_muF_by_T = np.sort(
            np.unique(
                np.concatenate(
                    (self.MSBar_data_container.mubar_by_T, muF_by_T_targets_extra)
                )
            )
        )
        ...
        ref_mu_by_T_choices = [
            2 * np.pi,
        ]  # TODO make this an input, read from somewhere else not hard code
        for reference_muF_by_T in ref_mu_by_T_choices:
            mu_0 = reference_muF_by_T
            g2_0 = self.MSBar_data_container.g2_MSBAR_spline(mu_0)

            these_target_mus = np.asarray(
                [
                    mu
                    for mu in target_muF_by_T
                    if mu >= self.min_muF_by_T_where_we_measure
                ]
            )

            g2s = np.asarray(
                [self.calc_mu_and_g2(mu, g2_0, mu_0) for mu in these_target_mus]
            )
            g2_spline = scipy.interpolate.InterpolatedUnivariateSpline(
                these_target_mus, g2s, k=3, ext=2
            )
            self.list_of_data_containers.append(
                PertRunMSBarDataContainer(
                    these_target_mus, g2_spline, reference_muF_by_T, self.nloop
                )
            )

            lpd.save_columns_to_file(
                self.outputpath_data
                + "g2_MSBAR_runFromMu_"
                + lpd.format_float(reference_muF_by_T, 2)
                + ".txt",
                (these_target_mus, g2s),
                ["mu/T", "g^2"],
            )
...
def get_general_parameters():
    NTEXP = 2
    max_muF_by_T_in_flow_extr = ScaleFunctions.convert_sqrt8taufTByTau_to_muFByT(
        sqrt8taufTByTau=0.25, tauT=0.25
    )
    muUV_by_T_NLO = (
        4 * np.pi * np.exp(1 - np.euler_gamma)
    )  # TODO rename these according to the paper
    ...
    general_params = GeneralParameters(
        ...
        min_muF_by_T_in_flow_extr=ScaleFunctions.convert_sqrt8taufTByTau_to_muFByT(
            sqrt8taufTByTau=0.3, tauT=0.5
        ),
        max_muF_by_T_in_flow_extr=max_muF_by_T_in_flow_extr,
        max_muBar_by_T=np.fmax(
            muBar_by_muF_NLO * max_muF_by_T_in_flow_extr, muUV_by_T_NLO
        ),
