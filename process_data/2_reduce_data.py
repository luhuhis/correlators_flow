...
#function for bootstrap routine that computes the EE correlator normalized to G_norm. numerator data is first index, polyakov loop is second index of data.
def compute_EE(data):
    EE = numpy.mean(data[0], axis=0) / (-6* numpy.mean(data[1], axis=0))
    ...
    #for i in range(len(EE)):
        #for j in range(len(EE[0])):
            #EE[i,j] = EE[i,j] / lpd.norm_corr(tauT_imp[j]) * Ntau**4
        ...
        for j in range(Ntau_half):
            for k in range(Ntau_half):
                for l in range(len(data)):
                    EEj = data[0,l,i,j] / (-6 * data[1,l,i,j]) #* Ntau**4 / lpd.norm_corr(tauT_imp[j])
                    EEk = data[0,l,i,k] / (-6 * data[1,l,i,k]) #* Ntau**4 / lpd.norm_corr(tauT_imp[k]) 
...
EE_samples, EE, EE_err = bootstr.bootstr(compute_EE, EE_data, numb_samples=n_samples, sample_size=n_datafiles, conf_axis=1, return_sample=True, same_rand_for_obs=True)
