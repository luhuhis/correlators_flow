...
from latqcdtools.experimental import scales_quenched as sq
...
def main():
    ...
    for i, Nt in enumerate(Nts):
        ...
        # flow times
        tauFbyScale_arr.append(tauFbya2 / scalefunc(betas[i]) ** 2)  # tauF/r0**2 or tauF/t0
    ...
    if args.calc_cont:
        for i in range(nflow):
            fitparams, fitparams_err = bootstr.bootstr_from_gauss(fit_sample, data=data[i], data_std_dev=data_err[i], numb_samples=1000,
                                                                  sample_size=1, return_sample=False, args=[1 / Nts ** 2, data_err[i], [1, 0]],
                                                                  parallelize=False)
            cont[i, 0] = fitparams[1]
            cont[i, 1] = fitparams_err[1]
            slope[i, 0] = fitparams[0]
            slope[i, 1] = fitparams_err[0]
        numpy.savetxt(datafile, numpy.column_stack((tauFbyScale_arr[0][flowstart:flowend], cont, slope)),
                      header="tfT2 |_T=1.5Tc  g2_cont    err     slope     err")
    else:
        try:
    ...
    fig2, ax2, _ = lpd.create_figure(xlabel=r'$\frac{\tau_F}{' + plotlabel + r'}$', xlabelpos=(1.03, 0.05), ylabelpos=(0.1, 0.9),
                                     figsize=((16 / 9) * (3 + 3 / 8), 3 + 3 / 8 - 1 / 2.54))  # r'$\tau_F T^2\big|_{T=1.5T_c}$',
    ax2.set_title(r'$ g^2_\mathrm{gf}\equiv \frac{128\pi^2 \tau_F^2 \langle E \rangle}{24}$', y=0.6)
    ax2.errorbar(tauFbyScale_arr[0][flowstart:flowend], cont[:, 0], cont[:, 1], fmt='x-', lw=0.5, markersize=2, mew=0.3, capsize=1, label='cont', zorder=-1)
    for i, Nt in enumerate(Nts):
    ...
    fig3, ax3, _ = lpd.create_figure(xlabel=r'$N_\tau^{-2}$', xlabelpos=(1.03, 0.05), ylabelpos=(0.1, 0.9), figsize=(1.5 * (3 + 3 / 8), 3 + 3 / 8 - 1 / 2.54),
                                     UseTex=False)
    ...
    for i in range(flowend - 1, flowstart, -1):
        if i % 10 == 0 and i > min_idx:
            ...
            ax3.errorbar(xpoints, extrapolation_ansatz(xpoints, slope[i, 0], cont[i, 0]), fmt='--', lw=0.5, alpha=0.5,
                         color=lpd.get_color(range(nflow), i, min_idx, nflow - 1))
