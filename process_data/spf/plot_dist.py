...
def main():
    ...
    requiredNamed.add_argument('--nmax', help='what nmax to use. valid only for model 1,2.', choices=[1, 2, 3, 4, 5, 6], type=int, required=True)
    parser.add_argument('--constrain', help='force the spf to reach the UV limit at large omega', action="store_true")
    requiredNamed.add_argument('--nsamples', help='number of bootstrap samples to draw from the gaussian distribution', default=200, type=int)
    ...
    constrainstr = "s1" if not args.constrain else "s2"
    ...
    structure = numpy.loadtxt(inputfolder+"samples_structure_"+fileidentifier+".txt", dtype=int)
    ab = structure[index]
    ...
    fig, ax, plots = lpd.create_figure(ylabel=r'$n$', xlabel=xlabel, xlabelpos=(0.95, 0.2), ylabelpos=(0.02, 0.97), UseTex=False)

    _, bins, _ = ax.hist(samples, args.nbins)

    ax.set_title(str(len(bins))+' bins, '+fileidentifier)

    fig.savefig(outputfolder+"/"+args.corr+"_"+fileidentifier+"_"+args.obs+".pdf")
