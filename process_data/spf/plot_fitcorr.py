...
def parse_args():
    ...
    parser.add_argument('--ylims', nargs=2, type=float, default=(0.75, 1.25))
...
def main():
    ...
    for i in range(nfiles):
        if not numpy.isnan(xdata[i]).any():
            ax.errorbar(xdata[i] + i * 0.003, ydata[i], yerr=[errorsleft[i], errorsright[i]], fmt='|', color=args.colors[i], label=args.labels[i])
