...
def main():
    ...
    labels =      ("2_a_s2_alpha_4_100_1e-04",    "2_s2_alpha_a_5",    "2_s1_alpha_a_4",   "2_s1_alpha_a_5",     "2_s2_beta_a_4",    "2_s2_beta_a_5",    "2_s1_beta_a_4",    "2_s1_beta_a_5")
    ...
    for label in labels:
        try:
            file = inputfolder + "/"+label+"/"+"params_" + label + ".dat"
            print("read in", file)
            data = numpy.loadtxt(file)
        except OSError:
            print(label, "not found, skipping")
            kappas.append(numpy.nan)
            errorsleft.append(numpy.nan)
            errorsright.append(numpy.nan)
            continue
        kappas.append(data[0, 0])
        errorsleft.append(data[0, 1])
        errorsright.append(data[0, 2])

    print(kappas)
    print(errorsleft, errorsright)
    ...
    fig, ax, plots = lpd.create_figure(xlabel=xlabel, xlabelpos=(0.5, -0.1), ylims=(0, 5), UseTex=False)

    for i, label in enumerate(labels):
        ax.errorbar(kappas[i], pos[i], xerr=[[errorsleft[i]], [errorsright[i]]], color=colors[i], fmt='x-', fillstyle='none', markersize=5, mew=0.25, lw=0.8, elinewidth=0.5, capsize=1.2)
